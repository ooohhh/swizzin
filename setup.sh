...
function _preparation() {
  ...
  apt-get -qq -y --force-yes install whiptail git sudo curl wget lsof fail2ban apache2-utils vnstat tcl tcl-dev build-essential dirmngr apt-transport-https python-pip >> ${log} 2>&1
  ...
  git clone https://github.com/ooohhh/swizzin.git --branch sandbox /etc/swizzin >> ${log} 2>&1
